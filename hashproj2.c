...
// Rover pointer for Next-Fit: continues from last allocation position
static node_t *rover = NULL;
...
void initialize_heap(void) {
    ...
    // Initialize rover to start of free list
    rover = free_list;
...
/* -------------------------------------------------------------------
   umalloc: Allocate memory using Next-Fit algorithm
   
   Continues from last allocation position (rover) and wraps around.
   This is "First-Fit with continuation" as specified.
------------------------------------------------------------------- */
void *umalloc(size_t size) {
    if (!heap_initialized) {
        initialize_heap();
    ...
    // Start from rover (last allocation position)
    if (rover == NULL) {
        rover = free_list;
    }
    
    node_t *start = rover;
    node_t *current = rover;
    node_t *prev = NULL;
    
    // Find prev node for rover
    if (rover != free_list) {
        node_t *temp = free_list;
        while (temp && temp->next != rover) {
            temp = temp->next;
        }
        prev = temp;
    }
    
    // Search from rover with wraparound until we circle back to start
    do {
        if ((size_t)current->size >= aligned_size) {
            // Found suitable block
            size_t remaining = current->size - aligned_size;
            
            // Split if remainder is large enough (at least 8 bytes payload)
            if (remaining >= sizeof(node_t) + 8) {
                ...
                node_t *new_free = (node_t *)((char *)current + sizeof(header_t) + aligned_size);
                ...
                // Update rover to continue from new free block
                rover = new_free;
                ...
                // Update rover to next block, wrap to start if needed
                rover = current->next ? current->next : free_list;
    ...
    // No suitable block found after full circular search
    return NULL;
}
...
void ufree(void *ptr) {
    ...
    // Verify magic number (catches corruption and double-free)
    if (header->magic != MAGIC) {
        fprintf(stderr, "Error: Invalid magic number. Possible corruption or double free.\n");
        exit(1);
    }
    
    // Clear magic to catch future double-frees on this block
    header->magic = 0;
    ...
    // Check for double free (block already in free list)
    ...
    if (freed_block->next != NULL) {
        ...
        if (end_of_freed == (void *)freed_block->next) {
            // If rover points to the block being merged, update it
            if (rover == freed_block->next) {
                rover = freed_block;
            }
